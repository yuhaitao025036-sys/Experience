...
class WithDenseViewFunction(torch.autograd.Function):
    ...
    def backward(ctx, grad_sparse):
        ...
        # Restore st_* attrs stripped by save_for_backward
        ...
        # Check for symbolic grad from downstream backward
        ...
        # Convert sparse grad → dense grad
        ...
        # Register symbolic grad for the inner StMoe backward to find
        ...
        # Run inner backward through the saved autograd graph
        # (StMoe.backward, SparseToDense.backward)
        torch.autograd.backward(result_dense, grad_dense.data)

        # sparse_data.grad holds the gradient from the inner backward chain
        grad_sparse_data = sparse_data.grad

        # Build grad_input: place sparse grad values back at input positions
        if grad_sparse_data is not None and in_indexes[0].numel() > 0:
            # Check symbolic_grad_registry for symbolic grad from inner chain
            ...
            if symbolic_grad_sparse is not None and hasattr(symbolic_grad_sparse, "st_relative_to"):
                # Reconstruct dense grad_input from symbolic sparse grad
            ...
            # Register symbolic grad for upstream backward (e.g. Merge.backward)
        ...
        # Clean up references to inner graph
