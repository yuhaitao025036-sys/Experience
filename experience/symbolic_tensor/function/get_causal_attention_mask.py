...
def get_causal_attention_mask(
    ...
    # Lower-triangular causal mask: (seq_len, seq_len), broadcasts over batch
    tril = torch.tril(
        torch.ones(seq_len, seq_len, dtype=torch.bool, device=token_mask.device)
    )

    # Expand token_mask: (batch_size, seq_len) -> (batch_size, seq_len, seq_len)
...
if __name__ == "__main__":
    ...
    # Test 1: All tokens valid, 3-token sequence
    ...
    # Test 3: Batch of 2 with different masks
